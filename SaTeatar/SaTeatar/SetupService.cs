    ...
    public class SetupService
    {
        public void Init(SaTeatarBpContext context)
        {
            ...
            if (!context.Zone.Any(x => x.PozoristeId == 1))
            {
                ...
                //#4 Pozoriste mladih Sarajevo
                context.Zone.Add(new Zone() { Naziv = "Zona S", PozoristeId = 4, UkupanBrojSjedista = 20 });
                context.SaveChanges();
                context.Zone.Add(new Zone() { Naziv = "Zona D", PozoristeId = 4, UkupanBrojSjedista = 40 });
                context.SaveChanges();
                context.Zone.Add(new Zone() { Naziv = "Zona L", PozoristeId = 4, UkupanBrojSjedista = 120 });
                context.SaveChanges();
            ...
            if (!context.Ocjene.Any(x => x.PredstavaId == 1))
            {
                ...
                //#5 Kupac
                context.Ocjene.Add(new Ocjene() { KupacId = 5, PredstavaId = 4, Ocjena = 5, Opis = "...opis...", Datum = new DateTime(2021, 7, 1) });
                context.Ocjene.Add(new Ocjene() { KupacId = 5, PredstavaId = 8, Ocjena = 2, Opis = "...opis...", Datum = new DateTime(2021, 7, 2) });
                ...
                context.SaveChanges();
                var listaOcjena = context.Ocjene.ToList();
                ...
                foreach (var item in listaOcjena)
                {
                    ...
                    {
                        case 1:
                            item.Opis = "Uzas, zaobici!";
                            break;
                        case 2:
                            item.Opis = "Lose, ne preporucujem!";
                            break;
                        case 3:
                            item.Opis = "Oke, nista posebno!";
                            break;
                        case 4:
                            item.Opis = "Vrlo dobro, preporucujem!";
                            break;
                        case 5:
                            item.Opis = "Odlicno, sve preporuke!";
                            break;
                    }
